...
function ShiftForm({ date, shift, onSave, onCancel }) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-40 flex justify-center items-center z-50">
      <form onSubmit={handleSubmit} className="bg-white rounded-xl p-8 w-96 shadow-2xl transition-transform transform hover:scale-105">
        <h2 className="text-2xl font-semibold mb-6 text-indigo-700 flex items-center space-x-2">
          ...
          <div className="flex space-x-3">
            {defaultColors.map(c => (
              <button
                key={c}
                type="button"
                className={`w-10 h-10 rounded-full border-4 ${color === c ? 'border-indigo-700' : 'border-transparent'} transition-transform transform hover:scale-110`}
        ...
        <div className="flex justify-end space-x-4">
          <button
            type="button"
            className="px-5 py-2 rounded-lg border border-gray-300 text-gray-700 hover:bg-gray-100 flex items-center space-x-2 transition"
            onClick={onCancel}
          >
